<script>
	...
	function goToPage(p) {
		if (p >= 1 && p <= totalPages) {
			navDirection = p > currentPage ? 1 : -1;
			currentPage = p;
					...
					{#each paginatedBooks as book, i (book.id)}
						<div class="group perspective-1000 relative flex flex-col items-center">
							...
							<div class="relative w-full max-w-[280px] md:max-w-full">
								...
								<!-- Shelf Shadow -->
								<div
									class="absolute right-0 bottom-[-25px] left-0 z-0 h-8 bg-black/20 blur-lg"
								></div>
							</div>

							<!-- Info: Mobile (Static Below) vs Desktop (Hover Card) -->
							<div
								class="relative z-30 mt-6 w-full max-w-[280px] text-center
                                        md:pointer-events-none md:absolute md:top-0 md:left-1/2 md:mt-0 md:w-64 md:max-w-full
                                        md:-translate-x-1/2 md:-translate-y-[90%] md:opacity-0 md:transition-all md:duration-500
                                        md:ease-out md:group-hover:-translate-y-[105%] md:group-hover:opacity-100"
							>
								<!-- Desktop Interaction Bridge (Invisible) -->
								<div
									class="hidden md:absolute md:-bottom-8 md:left-0 md:block md:h-8 md:w-full"
								></div>

								<!-- Bookmark Card (Shared visuals for Mobile & Desktop) -->
								<div
									class="pointer-events-auto relative overflow-hidden rounded-sm rounded-b-xl border-t-4 border-[var(--color-camo-brown)]
                                           bg-[#f5e6d3] p-6 pb-10 shadow-xl after:absolute after:top-0 after:left-0 after:z-0
                                           after:h-full
                                           after:w-full after:bg-[url('https://www.transparenttextures.com/patterns/aged-paper.png')] after:opacity-50 md:rounded-t-md
                                           md:rounded-b-3xl md:border-t-4 md:border-[var(--color-camo-brown)]"
								>
									<!-- Decorative Ribbon/String Hole (Visible on Desktop & Mobile now) -->
									<div
										class="absolute top-[-10px] left-1/2 z-20 block h-4 w-4 -translate-x-1/2 rounded-full border-2 border-[#d4b483] bg-[#1a0f0a] shadow-inner"
									></div>

									<h3
										class="font-display relative z-10 mb-1 text-xl leading-tight font-bold text-[var(--color-deep-forest)] md:text-2xl"
									>
										{book.title}
									</h3>
									<p
										class="relative z-10 mb-4 text-sm tracking-wider text-[var(--color-camo-brown)] uppercase opacity-80 md:font-semibold"
									>
										{book.author}
									</p>
									...
									<div class="relative z-10 flex flex-col items-center gap-3">
										<p
											class="text-xl font-bold text-[var(--color-sunset-orange)] md:mb-2 md:text-2xl"
										>
											{book.price}
										</p>
										...
										<a
											...
											target="_blank"
											class="inline-flex w-full items-center justify-center gap-2 rounded-full bg-[var(--color-moss-green)] px-6 py-2.5 text-base font-bold text-white shadow-md transition-transform hover:scale-105 hover:bg-[var(--color-sunset-orange)] hover:shadow-lg md:w-auto"
										>
											<span>Eu Quero</span>
